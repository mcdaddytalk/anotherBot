...
module.exports = class LicenseCheckCommand extends BaseCommand {
    constructor() {
        super('licensecheck', 'license', ['lc'], [], 10, false, true, '');
    }

    async run(client, message, args) {
        await client.sequelize
            .query(`SELECT * FROM Guilds WHERE license = '${args[0]}'`)
            .then(async ([rows]) => {
                console.log(rows);
        ...
        await client.sequelize
            ...
            .then(async ([rows]) => {
                if (!rows || rows.length <= 0) {
                    ...
                    await client.sequelize
                        .query(`UPDATE Guilds SET active = 'true' where guildID = '${message.guild.id}'`)
                        .then(([rows]) => {
                            active = true;
                        })
