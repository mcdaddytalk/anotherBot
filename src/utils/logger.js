const { MessageEmbed, WebhookClient } = require("discord.js"),
    chalk = require("chalk"),
    { black, blue, blueBright, cyan, cyanBright, green, greenBright, grey, magenta, magentaBright, redBright, yellow } = chalk
    moment = require("moment"),
    nodeLogger = require("simple-node-logger")
    ee = require('./embeds.js'),
...
const convertToString = (level, content) => {
  ...
  let logstring = `${blueBright('HQKBotJr')} ${grey('|')} [${cyan(timestamp)}] ${grey('|')} [${levelString}] `;
  if (typeof content == "string") {
    console.log(logstring, content.split("\n").map(d => `${d}`.green).join(`\n${logstring} `))
...
const sendLogs = (level, content, data) => {
  ...
  switch (level) {
    ...
    case "error":
      ...
      // console.log(`[${cyan(timestamp)}] [${redBright(level)}] ${content} ${data ? ": " + data : ""}`);
