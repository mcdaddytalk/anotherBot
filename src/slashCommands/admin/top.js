...
module.exports = class TopSlashCommand extends BaseSlashCommand {
    ...
    async run(client, interaction, args) {
        ...
        const topEmbed = new MessageEmbed()
            ...
            .setColor('DARK_GOLD')
            .setDescription(leaderboard.map((item, i) => `\`${++i}  ${item.name.padEnd(20)} ${item.kills.toString().padStart(3, ' ')} kills ${item.deaths.toString().padStart(3, ' ')} deaths ${item.kd.toString().padStart(5,' ')} KD\``).join('\n'));
