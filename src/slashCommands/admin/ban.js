    ...
    async run(client, interaction) {
        try {
            ...
            const row = new MessageActionRow().addComponents(
                new MessageButton().setCustomId('continue').setEmoji('✅').setStyle('PRIMARY'),
                new MessageButton().setCustomId('cancel').setEmoji('❌').setStyle('PRIMARY')
            );
            ...
            collector.on('collect', button => {
                ...
                switch (button.customId) {
                    case 'cancel':
                        interaction.editReply(':x: Canceled!').then(m => setTimeout(() => m.delete(), 3000));
