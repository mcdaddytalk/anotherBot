...
module.exports = class WithdrawSlashCommand extends BaseSlashCommand {
    constructor() {
        super({
            name: 'withdraw',
    ...
    async run(client, interaction) {
        ...
        const currentAmount = client.currency.getBank(id);
        if (transferAmount > currentAmount)
            return interaction.reply(`Sorry ${interaction.user}, you only have ${currentAmount} in the bank.`);
        if (transferAmount <= 0)
            return interaction.reply(`Please enter an amount greater than zero, ${interaction.user}.`);
        client.currency.withdraw(id, transferAmount);
